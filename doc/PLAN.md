...
| **Phase 12** | 预置榜单重做：规则新增 `maxScore`（显式满分，负分=封底）与 `weight`（总榜权重，默认 1）；进度型新增 `frameValues` 按难度分层（advancementValues > frameValues > defaultValue，默认 task 20/goal 30/challenge 60）；六榜数值与总榜口径定稿见 SCORING_PLAN ✅ | 1 天 |
...
> **关于进度分层 + 预置榜单重做（Phase 12）**：合并为一个 Phase（用户决策：进度按难度分层、广度榜默认不启用；六榜数值与总榜口径定稿见 SCORING_PLAN.md）。① **榜级字段**：规则新增 `maxScore`（显式满分，正榜 800 / 负榜 -800 封底，score = min/max 截断）与 `weight`（总榜加权和权重，默认 1，见总榜组成注记）。② **进度分层**：advancement 型新增 `frameValues`（frame 名 → 分值），优先级 `advancementValues`（逐进度）> `frameValues`（按难度）> `defaultValue`（兜底）；帧信息来自进度元数据（26.2 验证：`AdvancementFrame` 已更名 `AdvancementType`（TASK/CHALLENGE/GOAL），经 `Advancement.display()` → `Optional<DisplayInfo>` → `DisplayInfo.getType().getSerializedName()` 取 "task"/"goal"/"challenge"）；帧映射由 CompatHelper 在服务器启动后构建静态缓存（服务端进度注册表启动后固定），引擎/调度器传参接入（纯 Java 隔离保持）。默认分层值 task 20 / goal 30 / challenge 60（26.2 实测满分 ~3620：task 91 / goal 10 / challenge 25，2026-08-13；root 进度无 display 走 defaultValue）。③ **广度榜**：默认不预置（用户 2026-08-13 拍板，见 SCORING_PLAN 3.5），count 能力随计分模式未来规划。

> **关于总榜组成（Phase 12 决策）**：总榜 = 各分榜积分的**加权和**，默认权重均为 1（规则新增 `weight` 字段，用户 2026-08-13，见 SCORING_PLAN 1.2）；惩罚榜负分直接计入；各榜满分在设计时统一 800（`maxScore` 显式满分，进度榜天然封顶 ~3620 除外），加权和量级横向可比。

> **关于计分模式扩展（未来规划，Phase 12 不实施）**：规则 `mode` 字段（stat 型）推迟——`linear`（默认，现状公式）/ `log`（`multiplier × log(1 + value÷divisor)`，cap 截断后取对数，抑制刷分收益）/ `sqrt`（`multiplier × sqrt(value÷divisor)`）/ `count`（已解锁统计项计数：匹配项累计值>0 每项 +multiplier；tiers/cap/divisor 忽略；RL 灵感 A2，广度榜自建参考每解锁一项 +2 分，幂等可算，首次钓鱼/附魔/造访维度均计入）。`mode ≠ linear` 时配置 `tiers` 报错（意图冲突）。推迟理由（用户 2026-08-13）：六榜全部 tiers、`maxScore` 已覆盖 log/sqrt 的刷分抑制动机、广度榜默认不启用故 count 无预置需求；count 需连带 `statType` 通配支持（`StatMatcher` 的 statType 支持 `"*"`，此前仅 statPath 支持）。落地时机：出现饱和计分需求或服主自建广度榜需求时，正交扩展（不影响现有 linear/tiers 路径）。
